...
async def upload_file(
    ...
    fileBase64: Optional[str] = Form(default=None),
    filename: Optional[str] = Form(default=None),
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
):
    # n8nからのBase64文字列を受け取る場合（fileBase64とfilenameが両方存在する場合）
    if fileBase64 is not None and filename is not None:
        # Validate file type
    ...
    # 通常のmultipart/form-dataファイルアップロード
    if file is None:
        raise HTTPException(status_code=400, detail="ファイルまたはfileBase64とfilenameが必要です")
